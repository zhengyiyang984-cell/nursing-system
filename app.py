...
st.set_page_config(page_title="2F 護理排班系統-專業核對版", layout="wide")
...
def get_staff_configs(file):
    ...
    for i in range(start_row + 1, len(df)):
        ...
        if name in ["", "nan", "None", "姓名/職級"] or "星期" in name: continue
            
        # 顯示名稱 (序號+姓名)
        display_name = f"{no} {name}".strip()
        
        # 抓取最後一天班別 (用於銜接 Selectbox 的預設值)
        last_day = "off"
...
if file_a and file_b:
    try:
        ...
        bg_vacation = {n: [""] * num_days for n in names}
        ...
        st.success(f"✅ 已辨識 {len(names)} 位人員，預班表已自動同步至背景。")

        # --- 2. 依照你要求的代碼修改：核對權限與銜接狀態 ---
        ...
        for i, n in enumerate(names):
            with cols[i % 4]:
                with st.container(border=True): # 加入外框讓視覺更分開
                    st.markdown(f"👤 **{n}**")
    ...
    st.info("👋 請上傳檔案 A 與 檔案 B。系統會自動完成背景同步並生成人員核對區。")
