...
st.set_page_config(page_title="2F 護理排班系統-強制同步版", layout="wide")
...
with st.sidebar:
    ...
    vac_file = st.file_uploader("2. 上傳檔案 B (預約假)", type=["xlsx", "csv"])
...
if vac_file:
    if st.button("🔄 點此將檔案 B 資料同步至下方表格", type="secondary"):
        try:
            ...
            st.success("✅ 資料已同步，請查看下方表格！")
        except:
            st.error("❌ 檔案 B 格式不正確")

st.subheader("📅 自訂休假與固定班別")
