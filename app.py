...
st.set_page_config(page_title="2F 護理排班系統-純排休公平版", layout="wide")
...
if file_a and file_b:
    try:
        ...
        if st.button("🚀 啟動自動排班", type="primary", use_container_width=True):
            ...
            # 【核心修改】只追蹤與計算由系統生成的純公休（off）天數，排除特休(v)與預約假(R)的干擾
            pure_off_counts = {n: 0 for n in full_time_names}
            ...
            for d in range(num_days):
                ...
                # 【公平平衡核心】依照「純公休(off)」次數由多到少排序
                random.shuffle(pool)
                pool.sort(key=lambda x: pure_off_counts[x], reverse=True)

                for shift in ["N", "E", "D"]:
                    qualified = [n for n in pool if shift in perm_final[n]]
                    for _ in range(max(0, target[shift])):
                        if qualified:
                            chosen = qualified.pop(0) # 讓目前纯休假最多的人先去上班
                            res[chosen][d] = shift
                            if chosen in pool: pool.remove(chosen)
                
                # 剩下沒班的人轉純休假 off，並累加純休假計數
                for n in pool: 
                    res[n][d] = "off"
                    pure_off_counts[n] += 1

            # 【終極公平校正機制】將未請特休者的純 off 與請特休者的純 off 做平衡，拉近非請假日的休假落差
            for _ in range(50):
                max_staff = max(full_time_names, key=lambda x: pure_off_counts[x])
                min_staff = min(full_time_names, key=lambda x: pure_off_counts[x])
                
                if pure_off_counts[max_staff] - pure_off_counts[min_staff] > 1:
                    swapped = False
                    for d in range(num_days):
                        if res[max_staff][d] == "off" and res[min_staff][d] in ["D", "E", "N"] and bg_vacation[max_staff][d] == "" and bg_vacation[min_staff][d] == "":
                            ...
                            if current_shift in perm_final[max_staff]:
                                ...
                                res[min_staff][d] = "off"
                                pure_off_counts[max_staff] -= 1
                                pure_off_counts[min_staff] += 1
                                swapped = True
                                break
            ...
            st.success("🎉 自動排班計算完成！已啟動【排除特休干擾之純公休平衡機制】。")
            ...
            st.download_button("📥 下載 Excel 結果", out.getvalue(), "Schedule_Final_Perfect.xlsx")
