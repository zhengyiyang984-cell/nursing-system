"""validator.py - 2F 護理排班系統 V8 Stable 規則檢查"""
...
def _req(requests, nurse, day, days):
    ...
    return row[day] if 0 <= day < len(row) else ""
...
def _check_manpower(schedule, names, manpower, issues):
    for d in range(len(manpower)):
        for shift in CLINICAL_SHIFTS:
            actual = _shift_count(schedule, names, d, shift)
            min_req = int(manpower[d].get(f"{shift}_min", 0) or 0)
            if actual < min_req:
    ...
    for nurse in names:
        for d in range(days):
            ...
            if req == SHIFT_R and cur != SHIFT_R:
                issues.append(_issue("預排休違規", nurse, d, cur, "R 是固定預排休，不能改成上班。", "error", "請恢復為 R。"))
            elif req in [SHIFT_D, SHIFT_E, SHIFT_N, SHIFT_M] and cur != req:
                issues.append(_issue("預排班違規", nurse, d, cur, f"此日預排為 {req}，目前卻是 {cur}。", "error", f"請恢復為 {req}，或回到預排休表調整需求。"))
    ...
    for nurse in names:
        ...
        for d in range(days):
            ...
            if (prev, cur) in FORBIDDEN_TRANSITIONS:
                issues.append(_issue("班別銜接違規", nurse, d, cur, f"{prev} 後不能接 {cur}。", "error", "請調整前後班別，例如改 off 或同班別延續。"))
            if prev == SHIFT_N and cur not in [SHIFT_N, SHIFT_OFF, SHIFT_R]:
                issues.append(_issue("大夜銜接違規", nurse, d, cur, "N 後只能接 N 或休假。", "error", "請將 N 後調整為 N/off/R。"))
            prev = cur
    ...
    for nurse in names:
        ...
        while d < days:
            ...
            if d + 1 >= days or row[d + 1] != SHIFT_N:
                issues.append(_issue("大夜規則", nurse, d, SHIFT_N, "大夜必須成組：N→N→off→off，目前出現單顆 N。", "error", "請改成連續兩天 N，或將此 N 改為其他合法班別。"))
                d += 1
                continue
            for rest_day in [d + 2, d + 3]:
                if rest_day < days and row[rest_day] not in REST_SHIFTS:
                    issues.append(_issue("大夜規則", nurse, rest_day, row[rest_day], "大夜後必須休兩天：N→N→off→off。", "error", "請將大夜後兩天調整為 off/R，或整組夜班重新安排。"))
            d += 2
    ...
    for nurse in names:
        ...
        if nurse in PART_TIME:
            ...
            if d_count != PARTTIME_DAYS:
                issues.append(_issue("兼職天數", nurse, None, PARTTIME_ALLOWED_SHIFT, f"兼職 D 班需剛好 {PARTTIME_DAYS} 天，目前 {d_count} 天。", "error", "請限制郭珍君只排 3+3+2+2 共 10 天 D。"))
            if invalid > 0:
                issues.append(_issue("兼職班別", nurse, None, "", "兼職只能排 D/off/R，不能排 E/N/M。", "error", "請將兼職非 D 班別改為 off 或移給全職。"))
            continue

        off_total = sum(1 for x in row if x in REST_SHIFTS)
        if off_total < MIN_FULLTIME_OFF_DAYS:
            issues.append(_issue("全職休假不足", nurse, None, "", f"全職每月至少 {MIN_FULLTIME_OFF_DAYS} 天休，目前 {off_total} 天。", "error", "請用休假較多者頂替此人的部分班別。"))

        longest = _longest_work_streak(row, history_streak.get(nurse, 0))
        if longest > MAX_CONTINUOUS_WORK:
            issues.append(_issue("連續上班過長", nurse, None, "", f"最多連續上班 {MAX_CONTINUOUS_WORK} 天，目前最長 {longest} 天。", "error", "請在連班中間插入 off/R。"))

        for start in range(0, days, 7):
            block = row[start:min(start + 7, days)]
            if len(block) >= 5 and not any(x in REST_SHIFTS for x in block):
                issues.append(_issue("每週休假不足", nurse, start, "", f"第 {start // 7 + 1} 週未安排休假。", "warning", "請該週至少安排 1 天 off/R。"))

        for d in _single_day_fragments(row):
            issues.append(_issue("一日碎班", nurse, d, row[d], f"{row[d]} 出現 1 天碎班。", "warning", "可改 off，或與前後同班別連成 2 天以上。"))
